<template>
  <div class="flex items-center justify-between gap-4">
    <Card class="hover:shadow-md transition-shadow cursor-pointer border-2 border-dashed border-amber-300 bg-amber-50/30 relative group w-full !rounded-2xl" @click="$emit('click', plannedExpense)">
    ...
    <Button 
      variant="ghost" 
      size="icon" 
      class="h-10 w-10 text-red-400 hover:text-red-500 hover:bg-red-50"
      @click.stop="$emit('delete', plannedExpense)"
...
defineEmits<{
  ...
  delete: [plannedExpense: PlannedExpense]
}>()
