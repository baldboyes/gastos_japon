...
const STORAGE_KEY = 'trip-daily-budget-display-mode'

// Currency formatting
const currencySymbol = computed(() => {
  if (!props.currency) return '$'
  const currencyInfo = CURRENCIES.find(c => c.code === props.currency)
...
const remaining = computed(() => {
  return props.dailyLimit - todaySpent.value
})

const percentage = computed(() =>
  calculateBudgetPercentage(todaySpent.value, props.dailyLimit)
)
